...
export const createDonation = async (donationData: any) => {
  if (!isDBAvailable()) {
    console.log('Firebase not configured, creating mock donation...');
  ...
  try {
    ...
    const docPromise = addDoc(collection(db as Firestore, 'donations'), {
      ...donationData,
...
export const getPrograms = async () => {
  ...
  try {
    const programsQuery = query(collection(db as Firestore, 'programs'));
...
export const getCauses = async (): Promise<Cause[]> => {
  if (!isDBAvailable()) return MOCK_CAUSES;

  try {
    const causesQuery = query(
      collection(db as Firestore, 'causes'),
      orderBy('createdAt', 'desc')
    );
...
export const getEvents = async (): Promise<Event[]> => {
  if (!isDBAvailable()) return MOCK_EVENTS;
...
export const getBlogPosts = async (): Promise<BlogPost[]> => {
  if (!isDBAvailable()) return MOCK_BLOG_POSTS;

  try {
    const postsQuery = query(
      collection(db as Firestore, 'blog_posts'),
...
export const getLiveStats = async () => {
  ...
  try {
    const statsQuery = query(collection(db as Firestore, 'live_stats'), limit(1));
