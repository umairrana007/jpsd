...
// 1. Add insightDetails state
if (!content.includes('insightDetails')) {
...
// 2. Fix tag mismatch (replace </div> closing the <button> at the map end)
// We look for the map that has the onClick setInsightDetails
content = content.replace(
  /<button key=\{i\} onClick=\{\(\) => setInsightDetails[\s\S]*?<\/div>\s*\}\)\)/g,
  (match) => {
     // Find the very last </div> before })) and change to </button>
     return match.replace(/<\/div>(\s*\}\)\))$/, '</button>$1');
  }
);

// 3. Add Modal and AnimatePresence if missing at the end
...
`;
  // Insert before the last two closing tags of the component
  content = content.replace(/(\s+<\/div>\s+<\/div>\s+<\/div>\s+<\/div>\s+);\s+}(\s+)?$/, (match, p1) => {
     return p1 + modalCode + '    </div>\n  );\n}';
  });
}

fs.writeFileSync(path, content);
console.log('Final Dashboard Assembly Complete');
